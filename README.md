...
it and click `motion_detector` to load the example.
...
```
make -f tensorflow/lite/micro/tools/make/Makefile generate_motion_detector_make_project TARGET=himax_we1_evb
```
...
1.  Go to the generated example project directory.
    ...
    ```
    cd tensorflow/lite/micro/tools/make/gen/himax_we1_evb_arc/prj/motion_detector/make
    ```
...
3.  After example build finish, copy ELF file and map file to image generate
    ...
    ```
    cp motion_detector.elf himax_we1_evb.map ../../../../../downloads/himax_we1_sdk/image_gen_linux_v3/
    ```
...
5.  run image generate tool, generate flash image file.
    ...
    *   Before running image generate tool, by typing `sudo chmod +x image_gen`
    ...
    ```
    image_gen -e motion_detector.elf -m himax_we1_evb.map -o out.img
    ```
...
```
tensorflow/lite/micro/tools/make/gen/sparkfun_edge_cortex-m4/bin/motion_detector.bin
```
...
```bash
make -f tensorflow/lite/micro/tools/make/Makefile test_motion_detector_test
```
...
follow the instructions in [motion_detector/train/README.md](https://github.com/tensorflow/tensorflow/tree/master/tensorflow/lite/micro/examples/motion_detector/train/README.md).
